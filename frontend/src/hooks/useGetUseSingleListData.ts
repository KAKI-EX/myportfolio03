...
import { memoShow, shoppingDatumShow } from "lib/api/show";
...
export const useGetUseSingleListData = () => {
  const { showMessage } = useMessage();

  const getSingleListData = useCallback(async (props: Props) => {
    const { setLoading, event, getValues, index, listValues, listSetValue, onListOpen } = props;
